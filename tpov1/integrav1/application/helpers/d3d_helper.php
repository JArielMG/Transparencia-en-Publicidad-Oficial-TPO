...
function generateCsv($data, $delimiter = ',', $enclosure = '"') {
       ...
       foreach ($data as $line) {
           $alinea = get_object_vars ($line);
           array_pop ( $alinea );
           array_pop ( $alinea );
           fputcsv($handle, $alinea, $delimiter, $enclosure);
       }
       rewind($handle);
       while (!feof($handle)) {
...
if (! function_exists('exportToCVSEncuestaD3D')) {
   function exportToCVSEncuestaD3D( ) {
      $tabla       = 'tab_encuestas';
      $filename    = "Encuestas_TPO_INAI";
      $CI =& get_instance();
      $CI->load->model("Tpo_model", "export");
      $CI->export->initialize( $tabla );        
      $data =  $CI->export->get();
      $out = generateCsv($data, $delimiter = ',', $enclosure = '"');
      $header = '"No. encuesta","Tipo encuestado","Interesado en:","Vive en:","Se enteró por:","Le gustó:",';
      $header = $header . '"Es útil:","Comentarios:","Fecha hora:","La recomendaría:"' . chr(13);
      $out = $header.$out;
